...
Write-Host "== DiscountHub Stage 67: click target check =="
...
foreach ($platform in $platforms) {
  ...
  if ($platform -eq "AliExpress WW") {
    if ($location -notmatch "rzekl\.com|rztekl\.com" -or $location -notmatch "aliexpress.*item") {
      Write-Warning "AliExpress WW click does not look like an Admitad product deep link."
    }
  }
...
Write-Host "Stage 67 click target check completed."
